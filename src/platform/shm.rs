...
use std::{
	borrow::BorrowMut,
	time::{Duration, SystemTime, UNIX_EPOCH}, sync::atomic::AtomicBool,
};
...
impl Default for SDL2Platform {
	fn default() -> Self {
		...
		let shm = match ShmemConf::new().size(std::mem::size_of::<SharedMemoryBuffer>()).flink("powder-game-2").create() {
			Ok(m) => m,
...
impl<'a> Platform<SDL2BitmapData<'a>> for SDL2Platform {
	...
	fn entry(&mut self) {
		...
		'running: loop {
			...
			for event in event_pump.poll_iter() {
				match event {
					...
					Event::MouseButtonDown {
						...
						x: _,
						y: _,
					} => match SDL2Platform::get_mouse_button(mouse_btn) {
						Some(btn) => {
							btn.heldNow = true;
							btn.gotPress = true;
						}
					...
					Event::KeyDown {
						...
						keymod: _,
						repeat: _,
					} => match SDL2Platform::get_key(keycode.unwrap()) {
						Some(k) => {
							k.heldNow = true;
							k.gotPress = true;
						}
			...
			// copy the data from the canvas into the shared memory buffer
			let pixels = self.canvas.read_pixels(Rect::new(0, 0, WINDOW_WIDTH as u32, WINDOW_HEIGHT as u32), PixelFormatEnum::ARGB8888).unwrap();
			let rawshm = self.shm.as_ptr() as *mut SharedMemoryBuffer;
			let arr = unsafe {
				std::slice::from_raw_parts(pixels.as_ptr() as *mut u32, WINDOW_WIDTH * WINDOW_HEIGHT)
			};
			unsafe {
				(*rawshm).data.copy_from_slice(arr)
			}
